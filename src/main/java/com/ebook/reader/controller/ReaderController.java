...
public class ReaderController {
    ...
    private void displayEpubPage(int pageIndex, boolean isBackNavigation, boolean saveHistory) {
        ...
        try {
            ...
            // ✅ Check if we need to restore scroll BEFORE changing currentPage
            ...
            if (epubWebView != null) {
                ...
                System.out.println("✓ Displayed EPUB page " + (pageIndex + 1));
            }
    ...
    private double getCurrentScrollPosition() {
        ...
        try {
            // ✅ Get scroll percentage instead of pixel value
            ...
            if (result instanceof Number) {
                ...
                System.out.println("→ Current scroll: " + (percentage * 100) + "%");
                return percentage;
            }
            return 0;
        } catch (Exception e) {
        ...
        new Thread(() -> {
            try {
                if (currentBook.getFileType().equalsIgnoreCase("EPUB") && epubWebView != null) {
                    // EPUB: Wait for scroll to stabilize
                    try {
                        Thread.sleep(100);
                    } catch (InterruptedException e) {
                        // Ignore
                    ...
                    Platform.runLater(() -> {
                        ...
                        try {
                            double progress = totalPages > 0 ? (double)(currentPage + 1) / totalPages * 100 : 0;
                            ...
                            System.out.println("✓ Saved EPUB: page " + (currentPage + 1) + ", scroll " + (scroll * 100) + "%");
                        } catch (SQLException e) {
                    ...
                    // ✅ PDF: Wait for scroll to stabilize, then save
                    try {
                        Thread.sleep(200); // PDF needs more time
                    } catch (InterruptedException e) {
                        // Ignore
                    }
                    ...
                    Platform.runLater(() -> {
                        try {
                            ...
                            System.out.println("✓ Saved PDF: page " + (currentPage + 1) + ", scroll " + (scrollPosition * 100) + "%");
                        } catch (SQLException e) {
    ...
    private void handleToggleTOC() {
        if (tocPane != null) {
            boolean isVisible = tocPane.isVisible();
            tocPane.setVisible(!isVisible);
            tocPane.setManaged(!isVisible);

            if (toggleTocButton != null) {
                toggleTocButton.setText(isVisible ? "Show TOC" : "Hide TOC");
            }
        }
    }
    ...
    /**
     * Load default settings (theme and font size) from database
     */
    private void loadDefaultSettings() {
        try {
            ...
            if (settings != null) {
                ...
                System.out.println("✓ Loaded default settings: theme=" + currentTheme + ", fontSize=" + currentFontSize);
            }
        } catch (Exception e) {
    ...
    /**
     * Save setting to database as default
     */
